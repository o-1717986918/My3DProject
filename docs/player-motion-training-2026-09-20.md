...
执行，4/4 直立，但强力出球为 0/4。旧的 agent 观测曾给出一次前进 1.639 m、
横移 0.445 m；加入 RSMP 服务器场景图真值后，同一类左侧正中执行实际最大前进
只有 0.502 m、横移 0.511 m、方向峰值速度 1.385 m/s。差异来自动作结束前球视觉
已经陈旧，不能再把 agent 末帧估计当作物理结果。
...
运行时遥测升级为 schema 3：每周期同时保存最终电机目标、未叠加踢球残差的真实
Walk 基底目标和动作时间。左侧正中 `60467` 影子执行得到 3,892 帧，其中 62 帧
为完整 DynamicPass 基底/叠加对。精确 CPU 一步校验在 64 个抽样对上可运行；近球
状态的下一步关节位置 MAE 中位数为 0.435°，优于不发送动作的 0.877°，根位置
误差中位数 0.148 cm、姿态误差 0.413°。这支持把投影状态用于一步训练诊断，但
将整段目标带开环回放仍会发散并跌倒，因此不能把目标带优化冒充闭环策略训练。

启动器现在可将完整 RSMP 日志直接写到 `/home/win98/rl_runs`，解析器从 full/diff
场景图恢复权威球轨迹。权威结果确认 `60467` 只有 0.502 m 斜向出球。又对三个
事后筛选候选做仅强制影子筛查：`38407` 前进 0.322 m、横偏 0.056 m、峰值
1.177 m/s且直立；`104745` 完整执行但 0 m；`193620` 未取得可用释放。它们均未
达到长传/解围要求，已从运行时代码撤下，不扩大默认或强制释放的新鲜度窗口。
...
同时修正 K2 对跌倒的过严处理：旧实现把触球后跌倒同时作为每周期 `-100`、立即
终止和目标成功硬否决。现在触球前跌倒仍终止；触球后的第一次跌倒只计一次 25
分有限成本，仿真继续观察球结果，并分别输出 `ball_target_success`（球结果）和
`target_success`（球结果 + 直立恢复）。强力出球可以带偶发跌倒，但只有净比赛收益
更好时才会在奖励上占优。
