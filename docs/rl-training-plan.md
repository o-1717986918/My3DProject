...
35°/45°、2 m、2.0 m/s 到达速度上已经各有 1/6 物理成功。45° 固定课程把 GPU
命中由 3/32 提高到 10/32，精确 CPU 由 1/6 提高到 2/6；30°、22.5°固定课程均
保留 2/6，其中 22.5° 达到 GPU 16/32、32/32 触球、零跌倒。但继续切到固定 15°
不能稳定复现成功，一条激进分支产生 13/32 命中同时带来 5/32 接触失败，另一条
稳定分支保持触球却为 0/32 命中，直接 0° 也失败。

这证明下一阶段不能继续把每个角度训练成互相覆盖的动作偏置。课程改为重叠目标分布：
从 22.5°父模型训练 15°--22.5°，通过同一批次中的已会目标保留正样本并迫使网络使用
16 个目标条件特征；随后只在保持命中、触球和恢复的前提下把下界推到 7.5°、0°，
最后降低到达速度。每一阶段从已完成 K2 run 的指定 checkpoint 续训，并在 manifest
中锁定父 run、checkpoint step 和树哈希。

第一轮 15°--22.5°重叠训练仍从 4/64 降到 2/64，说明仅改变采样范围不足。权重审计
给出直接证据：22.5° checkpoint 的 16 个新增目标输入行范数仅 0.01667，110 个继承
输入行范数为 32.40125，比例约 `5.15e-4`；重叠训练后比例反而降到 `4.60e-4`。
因此下一次只做一个低成本可证伪实验：保持稳定 22.5°父模型和 15°--22.5°范围，
将 PPO 学习率从 `1e-5` 提到 `1e-4`，仍由 adaptive-KL 约束。若目标行权重和留出
角度命中没有同步增长，立即停止调标量，改用冻结继承 actor 的小型目标适配器或有
目标标签的监督初始化。
