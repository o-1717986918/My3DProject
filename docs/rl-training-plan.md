...
| 学习目标传球 runner | `kick_policy_v3` 的 98→23 观测、推理、残差解码、限位和同周期回退已接通；对照启动器默认 shadow，可显式 active | 外部 r2 ONNX 加载/推理通过，但冻结评估仅 27/92 且 1 次跌倒；新方向 PPO 三种子只比动作银行高 4/384（1.04 个百分点）且有一个种子回退 | 不晋级主动控制；继续保留 ONNX/DAgger 路线和 shadow 数据面，比赛控制使用动作银行 |
...
4. 使用现有 `LearnedKickRunner` 收集 shadow paired trace，并补 companion manifest
   自动校验；只有监督模型先超过确定性 fallback，才进行有限受约束 PPO；
