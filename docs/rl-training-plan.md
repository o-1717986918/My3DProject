...
5. 当前最高价值训练目标是稳定高速前进/转身/横移和连续追球，再是可部署的目标
   条件短传、接球、射门/解围；
...
当前不应原样延长 K2-B 的旧 PPO 目标。修复 MJX-Warp 接触传感器和接触后恢复后，
第一轮 393,216-step 固定 2 m 正式训练已经达到 100% 正确脚触球、100% 恢复段完成、
零跌倒，但目标命中仍为 0%。精确 CPU 六入口相位复测也为 6/6 触球、0/6 命中，
而且最近目标距离中位数由初始化的 1.547 m 退化到 1.593 m。问题已经从“能否触球”
收敛为“能否控制出球方向和到达速度”。下一轮从零扩展的 K1-D 初始化重新开始，
显式惩罚目标横轴球速，并用一次性终点、最近点和最大前向进度指标选模；不会从已
退化的 checkpoint 继续堆步数。

接触球后的偶发跌倒不再作为单回合的无限代价或物理出球成功的硬否决：首次跌倒只
计一次有限 25 分代价，回合继续观察球结果，目标命中与最终直立恢复分开报告。接触
前跌倒仍立即终止；候选晋级看独立种子的命中收益、跌倒频率和恢复负担的整体权衡，
不要求不现实的绝对零跌倒。

横向速度奖励的首轮 1,966,080-step 实验显示“方向信号有效但直接从自然触球方向跳到
0° 仍过难”。中期 checkpoint 在精确 CPU 六入口相位上把最终横差中位数从初始化的
4.795 m 降到 4.109 m、最近目标距离从 1.547 m 降到 1.478 m，仍为 0/6 命中；末期
又退化，因此不会挂载或盲选最后 checkpoint。零残差动作在更接近自然出球方向的
35°/45°、2 m、2.0 m/s 到达速度上已经各有 1/6 物理成功。下一阶段据此采用有证据的
课程：先在 45°/2.0 m/s 建立非零成功，再逐级转向 30°、15°、0°，随后降低到达速度；
每一阶段可从已完成 K2 run 的指定 checkpoint 续训，并在 manifest 中锁定父 run、
checkpoint step 和树哈希。

2026-09-05 的开放检索进一步收敛了具体顺序。Booster T1 的公开 ICRA 2026
实现不是从近球踢球开始，而是先训练 1--4 m 长距离追球 teacher，再训练定向触球、
带噪 DAgger student 和约束强化阶段。本项目已在 `train_striker_teacher.py` 中加入
`ball_chase` 与 `directional_kick`，并把完美姿态释放改为可学习的有界释放：
阈值 `1.00 -> 0.80`、确认 `25 -> 5` 个 50 Hz 步、球位半径
`0.11 -> 0.14 m`、朝向 `0.10 -> 0.15 rad`。这不是取消安全门，而是把最后的
厘米级接近和步态相位交给连续策略学习。

行走与 striker trainer 现在共享 PPO 预算计算：总步数先对完整 epoch 取整，再把
`num_evals-1` 调整为 epoch 数的最近因数。原因是 Brax 会把每个评估区间独立向上
取整；不做这一步会出现 manifest 写 1.18M、实际进度跑到 1.97M 的静默超训。
...
| 能力 | 启用方式 | 证据 | 限制与结论 |
| --- | --- | --- | --- |
| `FastWalkV2` 高速前向 | 源码启动器默认挂载 transition-recovered 候选；可用 `APOLLO_ENABLE_FAST_WALK=0` 消融 | 父候选 600-cycle 有 57 个高速样本、0 GetUp；新候选与 RapidTurn 的 1800-cycle 组合为 39 个高速样本、1 次独立跌倒/5 个 GetUp 状态样本，父组合为 4 次/32 个 | 只负责长距离近纯前向，稳定步态承担包线外与冷却；按时间收益与恢复代价继续优化，不要求零跌倒 |
| `RapidTurnV1` 快速转向 | 源码启动器默认挂载；负 yaw 使用纯镜像分支 | 固定 ±0.75 rad/s 各 64/64 直立；600-cycle 7v7 左/右 42/34 样本、0 GetUp | 独立原地转向专家，不等于横向/全向移动；可用 `APOLLO_ENABLE_RAPID_TURN=0` 消融 |
| 分档目标传球动作银行 | 源码树 WSL 启动脚本默认传入 `--enable-parameterized-kick`；2 m 用稠密残差表，3.5/5 m 用独立程序化轨迹 | 2 m 表的精确评估三种子约 94.7%–96.3%；3.5/5 m 名义轨迹分别约 3.552/4.970 m，未跌倒；统一方向任务中动作银行相对单一 2 m 先验从 15/128 提高到 49/128 | 长档仍缺独立球位留出和服务器比赛证据，保持 experimental；未晋级 ONNX 默认只 shadow，不再遮蔽动作银行 |
| 程序化 `DribbleTouch` | 同一参数化踢球开关；AP 无传球提交时可发出短触 | 精确 MuJoCo 球位扰动 20/20；受控旧 7v7 场景的一次接触约前进 0.831 m；共享释放契约修复后的自然完整比赛首次执行并完成，但球恢复可见后的位移只有约 0.15 m | 资产、能力包线、决策和 C++ runner 均已挂载；旧脚本场景不能代表自然比赛质量，当前仍只有右脚、近零角度和一个 0.55 m 锚点，不等同于连续学习带球 |
...
| 学习目标传球 runner | `kick_policy_v3` 的 98→23 观测、推理、残差解码、限位和同周期回退已接通；对照启动器默认 shadow，可显式 active | 外部 r2 ONNX 加载/推理通过；v35 在真实服务器执行 2.046 m 请求并前进 1.052 m、方向误差 +4.70°、无跌倒，但接收超时；冻结评估仍仅 27/92 且 1 次跌倒 | active 路径已证实但不晋级默认主动控制；继续用 ONNX/DAgger 和 shadow 数据面修复距离及接球闭环，动作银行保留回退 |
| 目标动作安全拒绝 | 直接运行二进制/保守配置时参数化动作关闭，或请求超出共享包线 | 运动层返回 `RejectedTargetedKickHold` | 已实现且正确；不会把目标传球静默变成固定前踢 |

本阶段交付验证：

- Apollo C++ 构建成功，17/17 CTest 通过；训练目录完整 Python 测试继续作为交付门槛，
...
- v35 已在生产策略与真实服务器物理中产生一次 active ONNX 触球，但只完成
  1.052/2.046 m 且接收超时；紧随其后的自然 v36 保持约 69% 新鲜球位于对方半场，
  却没有 Ready/触球并从普通 Walk 进入 21 次独立起身，说明下一训练重点是完整
  接近—释放和基础步态稳定性，而非扩大同一个静态门限；
...
| 程序化短触轨迹 | `/home/win98/rl_runs/procedural-kick/` 与仓库 YAML | 已挂载为窄 `DribbleTouch` fallback；自然完整比赛已贯通执行链，但目前观测到的实际位移约 0.15 m | 保留为 fallback，并生成完整接近—释放—接触—恢复监督学习教师数据 |
...
- 学习过渡的起始平面速度门已与程序轨迹门独立；当前仍取 0.50 m/s，因为已有
  460 个接受态仅 2 个超过该值，最大约 0.57 m/s，不足以支持更大的上线包线；
- 对照比赛启动器默认使用 `APOLLO_LEARNED_KICK_MODE=shadow`；只有显式 active
  消融且命中上述窄包线时，模型才取得关节控制权；任何不匹配、非有限输出或
  运行失败均按执行器契约回退，learned-only 请求则安全拒绝，绝不伪装成固定前踢；
- v35 已在真实服务器完成一次 `LearnedKickExecute`：2.046 m 请求前进 1.052 m、
  方向误差 `+4.70°`、横向误差 0.087 m、无跌倒；传球生命周期最终为 Timeout，
  因此它证明接线有效，但同时证明距离与接收完成率仍不够；
...
2026-09-12 的正式 Apollo 契约 CPU 基线表明纯左右横移各 64 个 10 秒回合全部
直立，中位速度约 `0.42/0.48 m/s`；真实服务器锁存拦截也已在左侧重复挡住原生
中心保持会漏掉的固定射门。因此不单独训练“普通横移模型”。下一训练入口是现有
Walk 来不及覆盖的来球：先按交点距离和剩余时间分桶，再分别训练站立身体封堵、
左右低扑和高球扑救，并保留动作后的起身/回位接口。
...
1. 固化当前 2/3.5/5 m 程序化动作银行、显式 forward-contact 回退、动作结果反馈和
   近门柱门将提前出击；学习踢球继续 shadow，不以 runner 完成冒充物理成功；
2. 从现有成功回合生成完整“接近—刹停—触球—恢复”轨迹，补齐阶段标签、动作来源、
   触球时刻和失败 hard negative；
3. 行为克隆一个阶段条件 striker actor，再用 DAgger 收集偏离状态。2026-09-06
   修正后的 2 m 零残差为 24/128，旧 PPO checkpoint 为 21/128，因此停止无监督
   小残差长训；
4. 使用现有 `LearnedKickRunner` 收集 shadow paired trace，并补 companion manifest
   自动校验；把 v31--v35 的步态瞬态、遮挡取消和 1.052/2.046 m 欠力结果加入
   hard-negative/DAgger 数据；只有监督模型先超过确定性 fallback，才进行有限
   受约束 PPO；
5. 对 3.5/5 m 教师补独立球位留出和服务器物理结果，再扩方向、左脚和长射；
6. 并行继续 `FastWalkV2` 的漂移、制动、动作切换和方向域适配，用净到位时间减去
   恢复损失衡量收益；
...
同日完成的后续试验否定了直接上线该门控：第一版在开发集 8/20，但新采 23 条
只有 2/23；19 场重训后的向前强出球门控在第二批新采 22 条为 10/22，低于最佳
静态原型 `60467` 的 11/22。`60467` 进入 RCSS 强制影子后 4/4 直立、0/4 达到
2.5 m；RSMP 服务器场景图校正后的代表结果只有 0.502 m 前进且横偏 0.511 m。
因此新的紧接顺序是：

1. **已完成基础设施**：记录运行时逐周期 Walk 基底目标、动作阶段，并从 RSMP
   full/diff 场景图恢复服务器真值球轨迹；一步精确 CPU 对照可用，但长时开环目标
   带会发散，所以后续必须闭环训练；
2. 在真实 `0.50..0.68 m` 交接窗优化固定的“对齐—发力—恢复”基线，并加入左右
   偏置、进入速度和步态相位；
3. 固定动作能在 RCSS 重复触球后，再训练释放时冻结专家的门控；不再用旧开发集
   或比弱基线 `164464` 好来替代与当前最佳静态动作的比较；
4. 跌倒按有限比赛代价计入，并单独报告恢复时间、丢球和重新参赛时间；允许偶发
   跌倒，但任何额外风险必须由更高净到位/出球收益补偿。
