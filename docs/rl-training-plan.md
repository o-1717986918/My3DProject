...
| 能力 | 启用方式 | 证据 | 限制与结论 |
| --- | --- | --- | --- |
| `FastWalkV2` 高速前向 | 源码启动器默认挂载恢复候选；可用 `APOLLO_ENABLE_FAST_WALK=0` 消融 | 恢复候选 600-cycle 7v7 有 57 个高速样本、0 个 GetUp；旧候选同长度为 39/10 | 只负责长距离近纯前向，稳定步态承担包线外与冷却；长组合测试仍有跌倒，继续训练而非卸载 |
