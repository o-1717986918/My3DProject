...
GLOBAL_DIR = getattr(sys, "_MEIPASS", ".")


class MathOps:
    """
    General mathematical operations that are not directly available through
    NumPy.
    """
    ...
    def rel_to_global_3d(
        local_pos_3d: np.ndarray,
        global_pos_3d: np.ndarray,
        global_orientation_quat: np.ndarray,
    ) -> np.ndarray:
        """Convert a local 3D position using a global pose quaternion."""
    ...
    def target_rel_angle(pos2d, ori, target, is_rad=False):
        """Return target angle relative to a 2D position and orientation."""
        if is_rad:
            return MathOps.normalize_rad(
                atan2(target[1] - pos2d[1], target[0] - pos2d[0]) - ori
            )
        else:
            return MathOps.normalize_deg(
                atan2(target[1] - pos2d[1], target[0] - pos2d[0]) * 180 / pi - ori
            )
    ...
    def rotate_2d_vec(vec, angle, is_rad=False):
        ...
        return np.array(
            [cos_ang * vec[0] - sin_ang * vec[1], sin_ang * vec[0] + cos_ang * vec[1]]
        )
    ...
    def is_point_between(p: np.ndarray, a: np.ndarray, b: np.ndarray) -> bool:
        ...
        return np.all(np.minimum(a_2d, b_2d) <= p_2d) and np.all(
            p_2d <= np.maximum(a_2d, b_2d)
        )
    ...
    def distance_point_to_ray(
        p: np.ndarray, ray_start: np.ndarray, ray_direction: np.ndarray
    ):
        ...
        k = (
            rd[0] / ray_direction[0]
            if abs(ray_direction[0]) > abs(ray_direction[1])
            else rd[1] / ray_direction[1]
        )
    ...
    @staticmethod
    def closest_point_on_ray_to_point(
        p: np.ndarray, ray_start: np.ndarray, ray_direction: np.ndarray
    ):
    ...
    def intersection_segment_opp_goal(a: np.ndarray, b: np.ndarray):
        """Intersect 2D segment ``ab`` with the opponents' goal line."""
        vec_x = b[0] - a[0]
    ...
    def intersection_circle_opp_goal(p: np.ndarray, r):
        """
        Intersect a circle with the opponents' front goal line.

        Only y coordinates are returned because x is always 15.
        """
    ...
    def circle_line_segment_intersection(
        ...
        """Find the zero, one, or two circle/line-segment intersections.

        :param circle_center: The (x, y) location of the circle center
        :param circle_radius: The radius of the circle
        :param pt1: The (x, y) location of the first point of the segment
        :param pt2: The (x, y) location of the second point of the segment
        :param full_line: Search the full line instead of only the segment.
        :param tangent_tol: Tolerance for treating two intersections as tangent.
        :return: A sequence containing zero, one, or two intersection points.
        ...
        if discriminant < 0:  # No intersection between circle and line
            ...
            # For a segment, discard intersections that fall outside its ends.
            if not full_line:
            ...
            # A tangent produces the same location twice; return it once.
    ...
    def get_score_based_on_tanh(
        ...
        """
        Calculate a configurable score from a hyperbolic tangent function.
        ...
        return (
            vertical_scaling * np.tanh(input * horizontal_scaling + horizontal_shift)
            + vertical_shift
        )
